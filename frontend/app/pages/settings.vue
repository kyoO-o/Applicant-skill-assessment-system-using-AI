...
onMounted(async () => {
  ...
  if (route.query.gcal === "connected") {
    ...
    gcalError.value = "Google Calendar холбоход алдаа гарлаа. Дахин оролдоно уу.";
  }
});
...
<template>
  <div class="space-y-6">
    <section class="rounded-3xl border border-border bg-card px-6 py-6 shadow-sm">
      ...
      <h1 class="mt-2 text-3xl font-semibold tracking-tight">Ажлын орчны тохиргоо</h1>
    ...
    <section class="grid gap-4 lg:grid-cols-2">
      ...
      <Card class="rounded-3xl border-border shadow-sm">
          ...
          <div class="flex items-center gap-3">
            <div class="flex h-9 w-9 items-center justify-center rounded-xl bg-blue-50">
              <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect x="3" y="4" width="18" height="17" rx="2" stroke="#4285F4" stroke-width="1.8" fill="white"/>
                <path d="M3 9h18" stroke="#4285F4" stroke-width="1.8"/>
                <path d="M8 2v4M16 2v4" stroke="#4285F4" stroke-width="1.8" stroke-linecap="round"/>
                <rect x="7" y="12" width="4" height="3" rx="0.5" fill="#4285F4"/>
              </svg>
            </div>
            <div>
        ...
        <CardContent class="space-y-3">
          ...
          <div v-if="gcalLoading" class="flex items-center gap-2 text-sm text-muted-foreground">
            <Spinner class="h-4 w-4" />
            <span>Шалгаж байна...</span>
          </div>

          <template v-else>
            <div class="flex items-center justify-between rounded-2xl border border-border bg-muted/30 px-4 py-3">
              ...
              <Button
                ...
                @click="gcalConnected ? disconnectGoogleCalendar() : connectGoogleCalendar()"
              >
                {{ gcalWorking ? "..." : gcalConnected ? "Холболт цуцлах" : "Холбох" }}
              </Button>
            </div>

            <p class="text-xs text-muted-foreground">
              Холбосны дараа ярилцлага товлох үед Google Calendar-т автоматаар нэмэгдэнэ.
            </p>
          </template>
        </CardContent>
      </Card>
      ...
      <Card class="rounded-3xl border-border shadow-sm">
        <CardHeader>
          ...
          <CardDescription>Ярилцлага, шинэ горилогч болон сануулгын тохиргоо.</CardDescription>
        </CardHeader>
        <CardContent class="space-y-3">
      ...
      <Card class="rounded-3xl border-border shadow-sm">
        <CardHeader>
          ...
          <CardDescription>Дэлгэцийн загвар болон өнгөний тохиргоо.</CardDescription>
        </CardHeader>
        <CardContent class="space-y-3">
          <div class="rounded-2xl border border-border bg-muted/30 p-4 text-sm text-muted-foreground">
