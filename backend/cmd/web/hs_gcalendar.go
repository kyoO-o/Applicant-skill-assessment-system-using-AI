...
// Returns the Meet link (empty string when generateMeet is false).
func createGCalEvent(ctx context.Context, recruiter *userman.User, title, description, location, attendeeEmail string, start time.Time, generateMeet bool) (string, error) {
	...
	if evResp.ConferenceData.ConferenceID != "" {
		return "https://meet.google.com/" + evResp.ConferenceData.ConferenceID, nil
	}

	return "", fmt.Errorf("gcal Meet conference not returned (status=%q)", evResp.ConferenceData.CreateRequest.Status.StatusCode)
}
